...
function cube(matrix, type = 1) {
    if (type === 1) {
        ...
        for (let i = 0; i < matrix.length; i++) {
            ...
            const z4 = z(zZ)
            ctx.beginPath()
            ctx.moveTo(z1.x, canvas.height - z1.y)
            ctx.lineTo(z2.x, canvas.height - z2.y)
            ctx.moveTo(z1.x, canvas.height - z1.y)
            ctx.lineTo(z3.x, canvas.height - z3.y)
            ctx.moveTo(z1.x, canvas.height - z1.y)
            ctx.lineTo(z4.x, canvas.height - z4.y)
            ctx.stroke()
        }
    }
}

function triangle(matrix, type = 1) {
    if (type === 1) {
        for (let i = 0; i < matrix.length; i++) {
            let z1 = z(matrix[i])
            ctx.lineWidth = 1
